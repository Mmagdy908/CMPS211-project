...
public class Document implements Serializable {
    ...
    // New method that handles text operations as a single unit
    public Operation insertTextAsOperation(String parentId, String text, int userId, List<String> characterIds) {
        // This is now a facade that calls the existing insertText but returns the first operation
        // The actual implementation in TreeCRDT will handle tracking the operation as a single unit
        List<Operation> operations = content.insertText(parentId, text, userId, characterIds);
        if (operations != null && !operations.isEmpty()) {
            return operations.get(0); // Return the first operation as a reference
        }
        return null;
    }
